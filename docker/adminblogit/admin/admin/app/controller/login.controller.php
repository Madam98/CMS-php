...
class LoginController {
    ...
    public function index() {
        ...
        //header('Location: ' . Settings::PATH['view'] . '/auth/login.php');
        // header('Location: ' . Settings::PATH['view'] . '/auth/login.php');
    ...
    public function validate() {
        try {
            ...
            $authorizationEntity = $this->authorizationModel->getIdByUsername($_REQUEST['username_admin']);

            $user = new User();
            $oneuser = $user->getOne($authorizationEntity->getUserId());
            $status = $oneuser->getTypeOfUserId();
            ...
            if($status == 1){
                if (!$authorizationEntity) {
                    ...
                    if (!$passwordUtils->verify($_REQUEST['password'], $authorizationEntity->getPassword())) {
                        ...
                        $_SESSION['username_admin'] = $authorizationEntity->getUsername();
