...
class Authorization {
	...
	public function getOne($id) {
		try {
            $sql = "SELECT * FROM AUTHORIZATION WHERE authorization_id = ?";
			$stm = $this->pdo->prepare($sql);          
    ...
    public function getUsernameById($id) {
        try {
            $sql = "SELECT username FROM AUTHORIZATION WHERE user_id = ?";
			$stm = $this->pdo->prepare($sql);  
			$stm->execute(array($id));
	...
	public function delete($id) {
		try {
            $sql = "DELETE FROM AUTHORIZATION WHERE authorization_id = ?";
			$stm = $this->pdo->prepare($sql);			          
            $stm->execute(array($id));
		} catch (Exception $e) {
			die($e->getMessage());
